...
const transactionsController = {
  createTransactions: async (req, res) => {
    try {
      ...
      if (setData.is_paid) {
        ...
        const minStock =
          productData.rows[0].stock - parseInt(req.body.totalOrder, 10);
  ...
  deleteTransactions: async (req, res) => {
    try {
      ...
      const transactions = await transactionsModels.deleteTransactions(id);
  ...
  confirmPaid: async (req, res) => {
    try {
      ...
      const minStock =
        productData.rows[0].stock - transactionData.rows[0].total_order;
